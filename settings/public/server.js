...
async function autoLocate(force = false) {
  ...
  settings.location = { lat, lon, source: "ip" };
...
async function restartHotspot() {
  try { await execP("sudo systemctl restart mm-hotspot", 20000); } catch {}
}
...
async function deleteWifiProfilesForSsid(ssid) {
  ...
  if (!target) return;
  let out = "";
  try { out = await execP("sudo nmcli -t -f NAME,TYPE con show", 10000); } catch { return; }
  const lines = out.split("\n").filter(Boolean);
  for (const line of lines) {
    const [name, type] = line.split(":");
    if (type !== "802-11-wireless") continue;
    const n = name || "";
    if (n === target || n.endsWith(`-${target}`) || n.includes(target)) {
      try { await execP(`sudo nmcli connection delete ${shQuote(n)}`, 10000); } catch {}
    }
  }
}
...
function buildConfigJs(settings) {
  ...
  const customCalendarObjects = customCalendarFeeds
    ...
    .map((u) => `          { fetchInterval: 7 * 24 * 60 * 60 * 1000, symbol: "calendar-check", url: ${jsString(u.trim())} }`)
    .join(",\n");

  const allCalendarObjects = `          { fetchInterval: 7 * 24 * 60 * 60 * 1000, symbol: "calendar-check", url: ${jsString(holidayCalendarUrl)} }${customCalendarObjects ? `,\n${customCalendarObjects}` : ""}`;
  ...
  function toJsArray(arr, fallback) {
    const cleaned = arr.filter((c) => typeof c === "string" && c.trim()).map((c) => `            ${jsString(c.trim())}`).join(",\n");
  ...
  const weatherModules = hasLatLon ? `,
    { module: "weather", position: "top_right", config: { weatherProvider: "openmeteo", type: "current", lat: ${lat}, lon: ${lon} } },
    { module: "weather", position: "top_right", header: "Weather Forecast", config: { weatherProvider: "openmeteo", type: "forecast", lat: ${lat}, lon: ${lon} } }` : "";

  return `/* Auto-generated by MagicMirror Setup\n * Generated: ${new Date().toISOString()}\n */\nlet config = {
  address: "0.0.0.0", port: 8080, basePath: "/", ipWhitelist: [], useHttps: false, httpsPrivateKey: "", httpsCertificate: "",
  language: "en", locale: "en-US", logLevel: ["INFO", "LOG", "WARN", "ERROR"], timeFormat: 24, units: "metric",
  modules: [
    { module: "alert" },
    { module: "updatenotification", position: "top_bar" },
    { module: "clock", position: "top_left" },
    { module: "calendar", header: "Calendars", position: "top_left", config: { calendars: [\n${allCalendarObjects}\n        ] } },
    ${complimentsModule}${weatherModules},
    { module: "newsfeed", position: "bottom_bar", config: { feeds: [\n${feedObjects || `          { title: "BBC News", url: "https://feeds.bbci.co.uk/news/rss.xml" }`}\n        ], showSourceTitle: true, showPublishDate: true, broadcastNewsFeeds: true, broadcastNewsUpdates: true, updateInterval: 300000 } }
  ]
...
function writeMagicMirrorConfig(settings) {
  ...
  if (!mmDir) throw new Error("MagicMirror directory not found.");
...
app.get("/api/device/info", (req, res) => {
  const ip = getLocalIp();
  res.json({ ok: true, ip, hostname: os.hostname(), settingsUrl: ip ? `http://${ip}:${PORT}` : null, settingsPath: SETTINGS_PATH });
});
...
app.post("/api/settings", (req, res) => {
  try {
    ...
    if (req.body?.wifi) {
      ...
      next.wifi = { ssid: incomingSsid, password: incomingPassword !== "" ? incomingPassword : (current.wifi?.password || "") };
    }
    if (req.body?.location) {
      const lat = normalizeCoordinate(req.body.location.lat);
      const lon = normalizeCoordinate(req.body.location.lon);
      next.location = { lat, lon, source: req.body.location.source || (lat != null && lon != null ? "user" : current.location?.source || null) };
    }
    if (req.body?.holidayCountryCode) next.holidayCountryCode = String(req.body.holidayCountryCode || "gb").toLowerCase();
    if (Array.isArray(req.body?.calendarFeeds)) next.calendarFeeds = req.body.calendarFeeds;
    if (Array.isArray(req.body?.newsFeeds)) next.newsFeeds = req.body.newsFeeds;
...
app.get("/api/wifi/scan", (req, res) => {
  exec("sudo nmcli -t -f SSID,SIGNAL dev wifi list ifname wlan0", (err, out) => {
    ...
    const seen = new Set();
    const nets = out.split("\n").filter(Boolean).map((line) => {
      const [ssid, signal] = line.split(":");
      return { ssid: (ssid || "").trim(), signal: Number(signal || 0) };
    }).filter((n) => n.ssid.length > 0 && !seen.has(n.ssid) && seen.add(n.ssid)).sort((a, b) => b.signal - a.signal);
...
app.post("/api/apply", async (req, res) => {
  ...
  if (!ssid || !password) return res.status(400).json({ ok: false, error: "Missing SSID or password" });
  ...
  setTimeout(async () => {
    try {
      ...
      try { await execP("sudo systemctl stop mm-hotspot", 15000); } catch (e) {}
      try { await execP("sudo nmcli con down Hotspot", 8000); } catch (e) {}
      try { await execP("sudo nmcli connection delete Hotspot", 8000); } catch (e) {}
      await deleteWifiProfilesForSsid(ssid);
      try { await execP("sudo nmcli dev disconnect wlan0", 8000); } catch (e) {}
      await new Promise((resolve) => setTimeout(resolve, 3000));
      try { await execP("sudo nmcli dev wifi rescan ifname wlan0", 10000); } catch (e) {}
      await new Promise((resolve) => setTimeout(resolve, 4000));
      setApply("wifi", `Connecting to ${ssid}...`);
      await execP(`sudo nmcli dev wifi connect ${shQuote(ssid)} password ${shQuote(password)} ifname wlan0`, 45000);
      setApply("internet", "Checking internet...");
      if (!await waitForInternet()) throw new Error("No internet");
      setApply("config", "Writing MagicMirror config...");
      writeMagicMirrorConfig(readSettings());
      setApply("finalizing", "Disabling setup hotspot mode...");
      try { await execP(`sudo rm -f ${SETUP_FLAG}`, 8000); } catch (e) {}
